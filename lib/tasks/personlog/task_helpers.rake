namespace :personlog do
  ...
  def warn_user_is_not_personlog
    unless @warned_user_not_personlog
      current_user = run("whoami").chomp
      unless current_user == "personlog"
        puts "#{Colored.color(:black)+Colored.color(:on_yellow)} Warning #{Colored.extra(:clear)}"
        puts "  You are running as user #{current_user.magenta}, we hope you know what you are doing."
        puts "  Things may work\/fail for the wrong reasons."
        puts "  For correct results you should run this as user #{"personlog".magenta}."
        puts ""
      end
      @warned_user_not_personlog = true
    end
  end
end
